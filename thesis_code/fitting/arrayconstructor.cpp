...
double * getNextRowOfFile(FILE * file, const int numberOfColumns) {	
	...
	char ** stringArray = (char **)malloc(sizeof(char*) * numberOfColumns);		// Array to hold string values before conversion to floats.
	char line [BUFFER_SIZE];	
		// Get our line.
	do {
	...
	while (line[i] != '\0') {		// until EOF:
		if (line[i] == delimeter || line[i] == '\n') {	// If it is the delimeter or endline, it is the end of a clump.
			if(i == 0 && line[0] == delimeter){i++; continue;}
...
//cout <<"number of rows is " <<numberOfRows <<endl;
	for (int n=0; n<numberOfRows;n++){
		row = getNextRowOfFile(file, numberOfColumns);
//		cout <<"get row success" <<endl;
		finalArray[n]=row;
//		cout <<"final array row " <<n <<" success" <<endl;
	}
