...
Fit::Fit(int nRows, int numberOfBins, datasource_t ds){
	nrphi0 = nRows;
	nBins = numberOfBins;
	...
	for(int i = 0; i< nBins; i++){
		if(density[i][0]!=0){
			logDensityData[i]=log10(DENSITY_SCALE_FACTOR*density[i][0]);	// scale, "log", then assign data from 2D array to 1D array
			}
...
fittingOutput_t Fit::makeFittingFileNames(char * ID, char * phi0){
	...
	strcat(scaledLogRadiusfilename, ID);
	...
	strcat(phi0LogDensityfilename, ID);
	...
	strcat(scaledLogDensityfilename, ID);
...
energyFittingOutput_t EnergyFit::makeFittingFileNames(char * ID){
